...
function checkPath(e: WheelEvent, content, dialog) {
  ...
  if (e.type === 'keydown' && path[0] === document.body) {
    ...
    if (dialog && hasScrollbarInOverlay(dialog) && isInside(selected, dialog)) {
      return !shouldScrollInOverlay(dialog, e)
    }
    return true
  }
  ...
  for (let index = 0; index < path.length; index++) {
    ...
    if (hasScrollbarInOverlay(el as Element)) return !shouldScrollInOverlay(el as Element, e)
  }

  return true
}

function shouldScrollInOverlay(el: Element, e: WheelEvent): boolean {
  ...
  if ((alreadyAtStart || alreadyAtEnd)) {
    return shouldScrollInOverlay(el.parentNode as Element, e)
  }
...
function hasScrollbarInOverlay(el?: Element) {
