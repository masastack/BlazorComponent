...
namespace BlazorComponent
{
    public abstract partial class BTreeItem<TItem> : BDomComponentBase
    {
        ...
        public BTree<TItem> TreeComponent { get; set; }

        /// <summary>
        /// 上一级节点
        /// </summary>
        [CascadingParameter(Name = "Node")]
        public BTreeItem<TItem> ParentNode { get; set; }
        ...
        /// </summary>
        /// <returns></returns>
        public List<BTreeItem<TItem>> GetParentNodes()
        {
        ...
        public BTreeItem<TItem> GetPreviousNode()
        {
        ...
        public BTreeItem<TItem> GetNextNode()
        {
            var parentNodes = GetParentNodes();
            var index = parentNodes.IndexOf(this);
        ...
        public void MoveInto(BTreeItem<TItem> BTreeItem)
        {
            if (BTreeItem == this || this.DataItem.Equals(BTreeItem.DataItem)) return;
            var parentChildDataItems = GetParentChildDataItems();
            parentChildDataItems.Remove(this.DataItem);
            BTreeItem.AddChildNode(this.DataItem);
        }
        ...
        {
            var tn = ChildNodes.FirstOrDefault(BTreeItem => BTreeItem.DataItem.Equals(dataItem));
            if (tn != null)
            {
                this.Expand(true);
                tn.SetSelected(true);
