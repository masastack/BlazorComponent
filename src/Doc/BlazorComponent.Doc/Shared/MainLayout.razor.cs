...
namespace BlazorComponent.Doc.Shared
{
    public partial class MainLayout : LayoutComponentBase, IDisposable
    {
        ...
        protected override async Task OnAfterRenderAsync(bool firstRender)
        {
            if (firstRender)
            {
                await JsInterop.InvokeVoidAsync("window.BlazorComponent.DocSearch.init", CurrentLanguage);
            }
        }
        ...
        {
            await JsInterop.InvokeVoidAsync("window.BlazorComponent.DocSearch.localeChange", culture.Name);
